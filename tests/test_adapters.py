...
from skillloop.adapters.hermes import (
    list_hermes_state_sessions,
    load_hermes_state_db,
    normalize_hermes_export,
)
...
def test_hermes_state_db_loads_latest_session_read_only(tmp_path):
    ...
    conn.execute(
        "INSERT INTO messages (session_id, role, content, tool_calls, timestamp, active) VALUES ('new', 'user', 'hello', NULL, 2.1, 1)"
    )
    conn.execute(
        "INSERT INTO messages (session_id, role, content, tool_calls, timestamp, active) VALUES ('new', 'assistant', 'hi', ?, 2.2, 1)",
        (json.dumps([{"name": "terminal", "arguments": {"command": "pwd"}, "success": True}]),),
    )
    conn.commit()
    conn.close()
