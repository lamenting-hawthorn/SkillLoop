...
def test_sft_export_uses_message_format_with_provenance():
    ...
    records = export_sft_records([trace], evaluations_by_trace={trace.id: evaluation}, proposals_by_trace={trace.id: [proposal]})

    assert records[0]["messages"] == [{"role": "user", "content": "hello"}, {"role": "assistant", "content": "hi"}]
...
def test_split_records_and_stats_are_deterministic():
    records = [{"metadata": {"trace_id": str(index)}, "messages": [{"role": "user", "content": "hello"}]} for index in range(10)]
    ...
    assert {name: len(items) for name, items in split_a.items()} == {"train": 6, "validation": 2, "test": 2}
...
def test_manifest_contains_export_metadata_and_provenance(tmp_path):
    trace = AgentTrace(source="test", messages=[AgentMessage(role="user", content="hello"), AgentMessage(role="assistant", content="hi")])
    evaluation = evaluate_trace(trace)
